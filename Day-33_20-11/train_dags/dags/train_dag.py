...
def prepare_data(**context):
    ...
    print(
        f"X_train['daily_consumption_load'] type: {type(X_train['daily_consumption_load'])}"
    )
    ...
    ti = context["ti"]
    ti.xcom_push(
        key="X_train", value=str(X_train_values)
    ...
    return "Data prepared successfull, greatt"
...
def train_model(**context):
    ...
    metrics = {"rmse": float(rmse), "mae": float(mae), "r2_score": float(r2_score_1)}
...
def validate_training(**context):
    ...
    print(
        f"Model Metrics - RMSE: {metrics['rmse']:.4f}, MAE: {metrics['mae']:.4f}, R2: {metrics['r2_score']:.4f}"
    )
